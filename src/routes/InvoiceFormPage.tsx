...
import { useCreateInvoice, useInvoice, useNextInvoiceNumber, useUpdateInvoice } from '../lib/invoices'
...
function itemsToDrafts(items: Invoice['items']): LineItemDraft[] {
...
export function InvoiceFormPage() {
  ...
  const { data: invoice, isLoading: isInvoiceLoading } = useInvoice(isEdit ? businessId : undefined, invoiceId)
  const { data: nextNumber, isLoading: isNextNumberLoading } = useNextInvoiceNumber(isEdit ? undefined : businessId)

  if (!businessId) return null
  const isReady = isEdit ? !isInvoiceLoading && Boolean(invoice) : !isNextNumberLoading
    ...
    <>
      ...
      {!isReady ? (
        ...
        <InvoiceForm
          ...
          invoice={invoice}
          defaultNumber={nextNumber?.number}
        />
      )}
    </>
  )
...
function InvoiceForm({
  ...
  const updateInvoice = useUpdateInvoice(businessId)

  const [clientId, setClientId] = useState(invoice?.clientId ?? '')
  const [number, setNumber] = useState(invoice?.number ?? defaultNumber ?? '')
  const [issueDate, setIssueDate] = useState(
    invoice ? invoice.issueDate.slice(0, 10) : new Date().toISOString().slice(0, 10),
  )
  const [dueDate, setDueDate] = useState(invoice ? invoice.dueDate.slice(0, 10) : '')
  const [terms, setTerms] = useState(invoice?.terms ?? 'Due within 30 days')
  const [items, setItems] = useState<LineItemDraft[]>(itemsToDrafts(invoice?.items))
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      if (isEdit && invoice) {
        ...
        navigate(`/businesses/${businessId}/invoices/${created.id}`)
      }
    } catch (err) {
      ...
      <Panel title="Invoice details">
        <div className="flex flex-col gap-4 px-5 py-5">
          ...
          <TextField
            ...
            required
            placeholder="Auto-assigned unless you set one"
            value={number}
